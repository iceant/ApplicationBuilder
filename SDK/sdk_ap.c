...
static sdk_ap_t mk(int size) {
    assert(size > 0);
    sdk_ap_t z = SDK_CALLOC(1, sizeof (*z) + size);
    assert(z);
    z->sign = 1;
    z->size = size;
    z->ndigits = 1;
...
sdk_ap_t sdk_ap_div(sdk_ap_t x, sdk_ap_t y) {
    ...
    r = mk(y->ndigits);
    {
        sdk_xp_t tmp = SDK_ALLOC(x->ndigits + y->ndigits + 2);
...
long int sdk_ap_modi(sdk_ap_t x, long int y) {
    ...
    rem = (long)sdk_xp_toint(r->ndigits, r->digits);
    sdk_ap_free(&r);
    return rem;
}
...
sdk_ap_t sdk_ap_fromstr(const char *str, int base, char **end) {
    ...
    carry = sdk_xp_fromstr(z->size, z->digits, p, base, &endp);
    assert(carry == 0);
    normalize(z, z->size);
    if (endp == p) {
        endp = (char *)str;
