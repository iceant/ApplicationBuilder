#include <sdk_crc8.h>

static const unsigned char sdk_crc8__table[] = { // 0x97 Polynomial Table, 8-bit, sourcer32@gmail.com
         };

////////////////////////////////////////////////////////////////////////////////
////

uint8_t sdk_crc8(uint8_t crc, uint8_t* data, sdk_size_t size)
{


    while(size--)
    {
        crc = crc ^ *data++; // Apply Byte

        crc = sdk_crc8__table[crc & 0xFF]; // One round of 8-bits
    }

    return(crc);

}