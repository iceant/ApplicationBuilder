...
static int applyu(sdk_err_t op(sdk_mp_t, sdk_mp_t, sdk_mp_t), sdk_mp_t z, sdk_mp_t x,
                  unsigned long u) {
    unsigned long carry;
    {
        sdk_mp_t mp_z = tmp[2];
        carry = sdk_xp_fromint(nbytes, mp_z, u);
        carry |= mp_z[nbytes - 1] & ~msb;
        mp_z[nbytes - 1] &= msb;
    }
    op(z, x, tmp[2]);
    return carry != 0;
...
static int apply(sdk_err_t op(sdk_mp_t, sdk_mp_t, sdk_mp_t), sdk_mp_t z, sdk_mp_t x, long v) {
    {
        sdk_mp_t mp_z = tmp[2];
        if (v == LONG_MIN) {
            sdk_xp_fromint(nbytes, mp_z, LONG_MAX + 1UL);
            sdk_xp_neg(nbytes, mp_z, mp_z, 1);
        } else if (v < 0) {
            sdk_xp_fromint(nbytes, mp_z, -v);
            sdk_xp_neg(nbytes, mp_z, mp_z, 1);
        } else{
            sdk_xp_fromint(nbytes, mp_z, v);
        }
...
int sdk_mp_set(int n) {
    ...
    if (nbytes <= 16){
        ...
        tmp[0] = SDK_ALLOC(3*nbytes + 2*nbytes + 2);
        assert(tmp[0]);
    }
...
sdk_mp_t sdk_mp_new(unsigned long u) {
    sdk_mp_t z = SDK_ALLOC(nbytes);
    assert(z);
    sdk_err_t err = sdk_mp_fromintu(z, u);
    if(err!=SDK_EOK){
        return 0;
    }
    return z;
...
sdk_err_t sdk_mp_fromintu(sdk_mp_t z, unsigned long u) {
    unsigned long carry;
    assert(z);
...
sdk_err_t sdk_mp_fromint(sdk_mp_t z, long v) {
    assert(z);
    if (v == LONG_MIN) {
...
sdk_err_t sdk_mp_toint(sdk_mp_t x, long* result) {
    unsigned char d[sizeof (unsigned long)];
    assert(x);
    sdk_mp_cvt(8*sizeof d, d, x);
    if(result){
        *result = (long)sdk_xp_toint(sizeof d, d);
    }
    return SDK_EOK;
}

sdk_err_t sdk_mp_cvt(int m, sdk_mp_t z, sdk_mp_t x) {
    int fill, i, mbytes = (m - 1)/8 + 1;
    assert(m > 1);
    assert(x); assert(z);
    fill = sign(x) ? 0xFF : 0;
    if (m < nbits) {
        int carry = (int)((x[mbytes-1]^fill)&~ones(m));
...
unsigned long sdk_mp_tointu(sdk_mp_t x) {
    unsigned char d[sizeof (unsigned long)];
    assert(x);
    sdk_mp_cvtu(8*sizeof d, d, x);
    return sdk_xp_toint(sizeof d, d);
}

sdk_err_t sdk_mp_cvtu(int m, sdk_mp_t z, sdk_mp_t x) {
    int i, mbytes = (m - 1)/8 + 1;
    assert(m > 1);
    assert(x); assert(z);
    if (m < nbits) {
        int carry = (int)(x[mbytes-1]&~ones(m));
        for (i = mbytes; i < nbytes; i++){
            carry |= x[i];
        }
        memcpy(z, x, mbytes);
...
sdk_err_t sdk_mp_addu(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int carry;
...
sdk_err_t sdk_mp_subu(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int borrow;
    assert(x); assert(y); assert(z);
...
sdk_err_t sdk_mp_mulu(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    assert(x); assert(y); assert(z);
    memset(tmp[3], '\0', 2*nbytes);
    sdk_xp_mul(tmp[3], nbytes, x, nbytes, y);
...
sdk_err_t sdk_mp_divu(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    assert(x); assert(y); assert(z);
    {
...
sdk_err_t sdk_mp_modu(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    assert(x); assert(y); assert(z);
    {
        memcpy(tmp[1], y, nbytes);
        y = tmp[1];
...
sdk_err_t sdk_mp_add(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int sx, sy;
    assert(x); assert(y); assert(z);
    sx = sign(x);
    sy = sign(y);
...
sdk_err_t sdk_mp_sub(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int sx, sy;
    assert(x); assert(y); assert(z);
    sx = sign(x);
    sy = sign(y);
...
sdk_err_t sdk_mp_neg(sdk_mp_t z, sdk_mp_t x) {
    int sx;
    assert(x); assert(z);
    sx = sign(x);
...
sdk_err_t sdk_mp_mul(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int sx, sy;
    assert(x); assert(y); assert(z);
    sx = sign(x);
    sy = sign(y);
...
sdk_err_t sdk_mp_div(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int sx, sy;
    assert(x); assert(y); assert(z);
    sx = sign(x);
    sy = sign(y);
...
sdk_err_t sdk_mp_mod(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) {
    int sx, sy;
    assert(x); assert(y); assert(z);
    sx = sign(x);
    sy = sign(y);
...
sdk_err_t sdk_mp_addui(sdk_mp_t z, sdk_mp_t x, unsigned long y) {
    assert(x); assert(z);
    if (y < BASE) {
...
sdk_err_t sdk_mp_subui(sdk_mp_t z, sdk_mp_t x, unsigned long y) {
    assert(x); assert(z);
    if (y < BASE) {
...
sdk_err_t sdk_mp_mului(sdk_mp_t z, sdk_mp_t x, unsigned long y) {
    assert(x); assert(z);
    if (y < BASE) {
...
sdk_err_t sdk_mp_divui(sdk_mp_t z, sdk_mp_t x, unsigned long y) {
    assert(x); assert(z);
    if (y == 0){
        return SDK_EDIVZERO;
    }
...
sdk_err_t sdk_mp_modui(sdk_mp_t x, unsigned long y, unsigned long * result) {
    assert(x);
    if (y == 0){
        return SDK_EDIVZERO;
    }
...
sdk_err_t sdk_mp_addi(sdk_mp_t z, sdk_mp_t x, long y) {
    assert(x); assert(z);
    if (-BASE < y && y < BASE) {
...
sdk_err_t sdk_mp_subi(sdk_mp_t z, sdk_mp_t x, long y) {
    assert(x); assert(z);
    if (-BASE < y && y < BASE) {
        int sx = sign(x), sy = y < 0;
        if (sy){
...
sdk_err_t sdk_mp_muli(sdk_mp_t z, sdk_mp_t x, long y) {
    assert(x); assert(z);
    if (-BASE < y && y < BASE) {
        int sx = sign(x), sy = y < 0;
        if (sx) {
...
sdk_err_t sdk_mp_divi(sdk_mp_t z, sdk_mp_t x, long y) {
    assert(x); assert(z);
    if (y == 0){
        return SDK_EDIVZERO;
    }
...
sdk_err_t sdk_mp_modi(sdk_mp_t x, long y, long * result) {
    assert(x);
    if (y == 0){
        return SDK_EDIVZERO;
    }
    ...
    return sdk_mp_toint(tmp[2], result);
}
...
int sdk_mp_cmpi(sdk_mp_t x, long y) {
    ...
    if (sx != sy){
        ...
        sdk_mp_fromint(tmp[2], y);
        return sdk_xp_cmp(nbytes, x, tmp[2]);
    }
}

sdk_err_t sdk_mp_and(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) { bitop(&); }
sdk_err_t sdk_mp_or (sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) { bitop(|); }
sdk_err_t sdk_mp_xor(sdk_mp_t z, sdk_mp_t x, sdk_mp_t y) { bitop(^); }
...
sdk_err_t sdk_mp_fromstr(sdk_mp_t z, const char *str, int base, char **end){
    int carry;
    assert(z);
    memset(z, '\0', nbytes);
